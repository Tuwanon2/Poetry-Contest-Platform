...
const GeneralCompetitions = () => {
  ...
  useEffect(() => {
    const fetchContests = async () => {
      try {
        ...
        const generalContests = (response.data || []).filter(contest => {
          const levels = contest.levels || contest.Levels || [];
          return levels.some(level => 
            (level.level_name || level.name || '').includes('ประชาชน') ||
            (level.level_name || level.name || '').includes('ทั่วไป') ||
            (level.level_name || level.name || '').includes('บุคคลทั่วไป') ||
            (level.level_name || level.name || '').includes('ไม่จำกัด')
          );
        });
  ...
  const getStatusBadge = (contest) => {
    ...
    // ใช้ Theme สี Pastel แบบเดียวกับหน้าหลัก
